...
- (void)setupContentView {
    ...
    UIImage *image = [UIImage imageNamed:@"1_blurt.jpg"];
    ...
    icon.image = [UIImage imageNamed:@"1_blurt.jpg"];
    icon.layer.cornerRadius = self.iconHeight * 0.5;
    icon.clipsToBounds = YES;
    self.icon = icon;
    
    [self.icon sd_setImageWithURL:[NSURL URLWithString:self.imgUrl] placeholderImage:[UIImage imageNamed:@"1_blurt.jpg"]];
    
    [imageView sd_setImageWithURL:[NSURL URLWithString:self.imgUrl] placeholderImage:[UIImage imageNamed:@"1_blurt.jpg"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        // 20 左右 R  模糊图片
        [imageView setImageToBlur:imageView.image blurRadius:21 completionBlock:nil];
    }];
